...
# Join :- @Shubam_chitty #bgmiddoserpython
...
# Join :- @Shubam_chitty #from keep_alive import keep_alive
# Join :- @Shubam_chitty #keep_alive()
# Join :- @Shubam_chitty # insert your Telegram bot token here
bot = telebot.TeleBot('6638370999:AAFvxFqQZzyt3Qr0G7CNQyUiSZZYj_ZLLWw')

# Join :- @Shubam_chitty # Admin user IDs
admin_id = ["6543206935"]

# Join :- @Shubam_chitty # File to store allowed user IDs
USER_FILE = "users.txt"

# Join :- @Shubam_chitty # File to store command logs
LOG_FILE = "log.txt"


# Join :- @Shubam_chitty # List of proxy addresses
PROXIES = [
...
"https://134.73.65.97:6649"
    # Join :- @Shubam_chitty # Add more proxy addresses as needed
]
...
# Join :- @Shubam_chitty # Function to read free user IDs and their credits from the file
def read_free_users():
    try:
        with open(FREE_USER_FILE, "r") as file:
            lines = file.read().splitlines()
            for line in lines:
                if line.strip():  # Join :- @Shubam_chitty # Check if line is not empty
...
# Join :- @Shubam_chitty # Function to log command to the file
def log_command(user_id, target, port, time):
    ...
    with open(LOG_FILE, "a") as file:  # Join :- @Shubam_chitty # Open in "append" mode
        file.write(f"Username: {username}\nTarget: {target}\nPort: {port}\nTime: {time}\n\n")


# Join :- @Shubam_chitty # Function to clear logs
def clear_logs():
    try:
...
# Join :- @Shubam_chitty # Function to record command logs
...
# Join :- @Shubam_chitty # Function to handle the reply when free users run the /bgmi command
...
# Join :- @Shubam_chitty # Dictionary to store the last time each user ran the /bgmi command
bgmi_cooldown = {}

COOLDOWN_TIME =0

# Join :- @Shubam_chitty # Handler for /bgmi command
...
def handle_bgmi(message):
    ...
    if user_id in allowed_user_ids:
        ...
        command = message.text.split()
        if len(command) == 4:  # Join :- @Shubam_chitty # Updated to accept target, time, and port
            target = command[1]
            port = int(command[2])  # Join :- @Shubam_chitty # Convert time to integer
            time = int(command[3])  # Join :- @Shubam_chitty # Convert port to integer
            if time > 240:
            ...
            response = "✅ Usage :- /bgmi <target> <port> <time>"  # Join :- @Shubam_chitty # Updated command syntax
    else:
...
# Join :- @Shubam_chitty # Add /mylogs command to display logs recorded for bgmi and website commands
...
# Join :- @Shubam_chitty # Import the 'time' module for sleep functionality
import time
